...
- **Vision (comprendre l'image)** : GPT (OpenAI), Claude (Anthropic),
  ...
  qu'un VLM 6 Go local ([VLM local](vlm-local.md)),
...
- **Le support des frameworks** : LangChain/LlamaIndex abstraient le
  ...
  du domaine retrieval est exactement ce raisonnement, étendu aux modalités.
- **L'arbitrage, toujours le même** :
...
Une page de synthèse (README domaine multimodal ou veille Obsidian) : tableau
modalité × provider local × provider cloud × quand choisir quoi —
révisée à la veille, pas un
développement.
