...
Le compteur est monté, sous sa forme honnête : `Reponse.tokens_lus` et
`Reponse.tokens_generes` dans
[`llm/ollama.py`](../../src/framework/llm/ollama.py). Ce ne sont pas *nos*
comptes — c'est le serveur qui les rend, et la brique se contente de ne pas
les perdre. L'étape les affichait puis les jetait ; désormais le coût d'un
tour est une donnée que l'appelant peut lire.

Un vrai tokenizer côté client reste absent, et c'est délibéré : tant qu'on
parle à un serveur qui compte pour nous, l'écrire serait dupliquer son
vocabulaire — donc se tromper dès qu'on change de modèle. Il deviendra
nécessaire le jour où il faudra décider *avant* d'envoyer (plafonner un
prompt, découper un document), pas pour constater après.
