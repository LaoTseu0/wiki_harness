...
c'est ce qui permet au domaine retrieval d'utiliser le retrieval sans embarquer
la boucle d'agent.
...
- **Les six briques et leur origine** — chaque brique est un script du
  ...
  | mémoire | domaine agent (OKF) | `charger(clé) / sauver(clé, valeur)` |
  | retrieval | domaine retrieval (`rag_commun`) | `chercher(question, k, filtres) → chunks` |
  | evals | 07_evals du domaine retrieval | `évaluer(jeu, système) → scores` |
...
0.0.1 de la
[sortie précoce et semver](sortie-precoce-semver.md).
