...
Le référentiel que « connaître par cœur »
...
- **Les dix, avec l'ancrage sur ce parcours** (édition 2025) :
  ...
  |---|---|---|
  | LLM01 | Prompt injection | [prompt injection indirecte](../mcp/prompt-injection-indirecte.md) (démo) |
  | LLM02 | Sensitive info disclosure | traces self-hostées [langfuse self-hosté](langfuse-self-hoste.md), local-first |
  ...
  | LLM05 | Improper output handling | `calculer` filtré [function calling à la main](../fondamentaux/function-calling.md), sorties validées |
  | LLM06 | Excessive agency | moindre privilège [conteneur et moindre privilège](../agent/conteneur-moindre-privilege.md), lecture seule MCP |
  | LLM07 | System prompt leakage | pas de secret en prompt |
  | LLM08 | Vector/embedding weaknesses | filtres [filtres métadonnées](../retrieval/filtres-metadonnees.md), périmètres |
  | LLM09 | Misinformation | grounding + citations [RAG complet](../retrieval/rag-complet.md), evals hallucination |
  | LLM10 | Unbounded consumption | MAX_TOURS, plafonds, suivi coûts [suivi des coûts](suivi-des-couts.md) |

- **La lecture transversale** : la moitié du Top 10 se mitige par
  **moindre privilège + validation des E/S** — exactement les deux
  périmètres du domaine agent. Le dire ainsi montre qu'on a compris la
...
- **Ce qui reste ouvert (à assumer)** : le homelab n'a pas de gateway
 ...
 et dire pourquoi c'est hors
  scope à cette échelle.

## En pratique

Une page dans le README du domaine production : le tableau ci-dessus rempli avec
