...
**Mnémos** est la finalité : un assistant agentique complet, avant tout local, mono-utilisateur, auto-hébergé. Il route entre plusieurs agents et tient une mémoire de plusieurs natures. C'est le produit.

**Praxis** est la bibliothèque sur laquelle Mnémos est bâti. Elle ne connaît rien de Mnémos : elle expose des briques génériques — un client, une boucle, un registre d'outils, des mémoires, un routeur — que Mnémos compose et configure.
...
- **Intégration** — la brique Praxis de la couche, déposée dans `Praxis/`. La pièce comprise devient du code réutilisé.
...
« Complet » veut dire que Mnémos assure réellement ces capacités, pas une démo de chacune :
...
Mnémos n'est **pas** un service multi-tenant, une plateforme ou un produit à vendre. Il sert une personne, sur sa machine — cette limite fixe le périmètre.

## Praxis — la bibliothèque

Une brique par couche, déposée à la fin du Parcours qui en enseigne les mécanismes (section *Intégration*), dans `Praxis/`. Ce tableau est le contrat entre le cours et le framework.

| Brique Praxis | Rôle | Déposée au Parcours |
|---|---|---|
...
| `memory` | magasins (vectoriel, clé-valeur, graphe, wiki auto-écrit) + processus, rappel commun | 6 · La mémoire |
...
Ce qui est **hors** Praxis : la personnalité de Mnémos, ses prompts système, sa topologie d'agents concrète, le câblage de ses mémoires. Praxis fournit le routeur ; Mnémos décide qui sont les ouvriers.

## La mémoire — le cœur de Mnémos

La brique `memory` tient plusieurs `store` derrière une interface de rappel commune, plus les processus qui les maintiennent. Notes de travail dans [memoire.md](../raw/memoire.md) ; en bref :

- **Mémoire** — vectoriel (le sens), clé-valeur (l'état exact), graphe (entités et relations, faits à validité datée), wiki auto-écrit (connaissance auto-rédigée).
...
Praxis fournit les mémoires et les processus ; Mnémos décide ce qu'il consolide, et quand.

## Ce que ça impose au cours

1. **Chaque Parcours se termine par une *Intégration*** — la brique Praxis de sa couche. Le framework sédimente couche par couche, il n'est pas un chapitre final.
2. **Un Parcours de clôture assemble Mnémos** — non un mécanisme neuf, mais la composition des briques : topologie d'agents, câblage des quatre mémoires, voix et vision, persistance.
...
Chaque leçon s'ouvre sur un bloc de métadonnées. `promeut` pointe une brique réelle de `Praxis/`, et un contrôle échoue si elle ment. Le reste identifie et date : `id` (slug stable, unique), `type`, `titre`, `tags`, `created`, `updated`.
...
created: 2026-07-24
updated: 2026-07-24
promeut: Praxis/sampling.py       # ou : aucune — <raison en une ligne>
---
```
