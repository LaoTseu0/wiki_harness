...
> Refaire à la main chaque couche d'une application LLM — de la génération d'un token jusqu'à Mnémos, l'assistant multi-agent — puis déposer ce qu'on a compris dans **Praxis**, la bibliothèque maison sur laquelle Mnémos sera bâti. Terrain : un petit modèle local.
...
- les natures de mémoire — épisodique, sémantique, procédurale ; le travail est la fenêtre du P2
- les stores — vectoriel, clé-valeur, graphe, wiki auto-écrit
- embeddings ; similarité cosinus
- chunking ; indexation ; recherche top-k
- RAG complet — récupérer, injecter, répondre
- retrieval hybride — lexical (BM25), vectoriel, graphe
- GraphRAG — entités, relations et parcours multi-hop
- mémoire temporelle — validité, provenance, évolution des faits
- vecteur vs graphe — le sens proche contre le multi-hop et le temporel
- filtres de métadonnées ; Qdrant
- re-ranking — bi-encoder vs cross-encoder ; HNSW
- cycle de vie de la mémoire — écrire, scorer, retrouver, mettre à jour, consolider, oublier
- évaluation — rappel, précision, fidélité ; RAGAS / DeepEval
- RAG vs fine-tuning — quand ce n'est pas la réponse
- contamination de mémoire — empoisonnement et isolation
- Ajouter au glossaire — contextual retrieval, Graphiti / Kuzu

**Cas pratique** — construire un RAG, le mesurer, l'étendre avec une mémoire épisodique datée puis invalider un fait.
**Intégration** — `memory` : plusieurs stores derrière une même interface de rappel.
...
**Cas pratique** — brancher la voix ou la vision sur Mnémos.
...
Aucun mécanisme nouveau. On compose les briques Praxis en un assistant qui tourne :
...
**Cas pratique** — Mnémos est le cas pratique.
**Intégration** — Praxis atteint sa première version stable ; Mnémos tourne dessus.
